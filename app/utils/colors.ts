...
export const TRAFFIC_GLOW: Record<TrafficLevel, string> = {
  low: '0 0 12px #22c55e88',
  medium: '0 0 12px #f59e0b88',
  high: '0 0 16px #ef444488',
};
...
export function getTrafficColor(level: TrafficLevel) {
  return TRAFFIC_COLORS[level];
}

export function getTrafficGlow(level: TrafficLevel) {
  return TRAFFIC_GLOW[level];
}

export function buildLeafletIcon(level: TrafficLevel, selected = false) {
